...
if (!targetPath) {
  console.error("Usage: node gen-skeleton.mjs [--type|-t component|hook|feature] <path/to/name>");
  process.exit(1);
}
...
const generateFeature = async (target) => {
  ...
  await fs.writeFile(path.join(fullPath, 'components', 'index.ts'), '// Export feature components\n', 'utf-8');
  await fs.writeFile(path.join(fullPath, 'hooks', 'index.ts'), '// Export feature hooks\n', 'utf-8');
  await fs.writeFile(path.join(fullPath, 'stores', 'index.ts'), '// Export feature stores\n', 'utf-8');
  await fs.writeFile(path.join(fullPath, 'types', 'index.ts'), '// Export feature types\n', 'utf-8');
