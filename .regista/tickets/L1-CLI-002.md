...
### 1. 変更内容サマリー
- `package.json` の scripts に `"ticket:done": "tsx .regista/scripts/ticket-done.ts"` を追加
- root package に `tsx` を devDependencies として追加
- `Makefile` に `done` ターゲットおよび引数受渡しルール `%: @:` を追加、`help` ターゲットに `make done` の説明を追加

### 2. 検証結果
- [x] `rtk biome check package.json` パス
- [x] `make help` に `done` が表示されることを確認
- [x] `rtk pnpm ticket:done --help` でヘルプが表示されることを確認
