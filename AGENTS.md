...
- **Fast-Track Mode (軽量・単独実行モード - 原則こちらをデフォルト適用):**
  ...
  - **手詰まり防止ガードレール (HANDOVER ON IMPASSE):** Flash / Lowなどのモデルで実装中に要件のズレが生じ、要件調整を重ねても2往復以上意図が噛み合わず手詰まりになった場合、無理に同一チャットで修正を繰り返してコンテキスト（トークン）を浪費してはならない。エージェントは自律的に「**引継ぎ**」を提案するか、ユーザーからのキーワード「**引継ぎ**」を受けて即座に別チャット引き継ぎプロトコル（セクション8）を実行すること。
...
- **チケット出力・運用プロトコル (DAG & Markdown Distribution Standard):**
  - **1. 分散Markdownファイル管理 (`.regista/tickets/[ID].md`):**
    ...
    - **ファイル分割設計図の明記:** GMがタスクをチケット化する際、単なる機能要件だけでなく、「このチケットでは、これら〇〇のファイルを作成（または分割）する」という具体的なファイル構成案（純粋関数と状態の分離など）をチケットの本文（ContextやSpec）に必ず明記すること。
  ...
  - **5. チャットへの厳格なチケット一覧表出力 (`.regista/templates/board-summary.md` 準拠):**
    ...
    - 各行のTicket IDには、レイヤー深度ごとに必ず異なる色のサークル絵文字（同一レイヤーには同色、異なるレイヤーには直前のレイヤーと異なる色）を付与する。
      ...
      - `⚫ L8` (L7完了待ち)
      - `⚪ L9+` (深層ブロック)
  - **5.5. タスク着手時のインジケーター表示とサイレント実行規約 (Silent Execution & Indicator):**
    - **着手時のインジケーター表示**: チケット（またはAAWU）着手時、チャットへの出力は「今実行しているタスク」を示す1行のインジケーターのみとする。
      - 形式: `⏳ [RUNNING] {Ticket ID}: {タスク名}`
    - **作業中の実況中継・コマンド発言の厳格禁止 (Silent Execution)**: ユーザーはどのコマンドを実行しているかなどは気にしておらず、ただ結果のみを待っている。「〇〇コマンドを実行します」「型チェック中...」等の途中経過やおしゃべりをチャットへ出力してはならない。ツール呼び出しにより裏で静かに実行を完遂すること。
  - **6. チケット実装完了時の超凝縮チャット報告フォーマット (`.regista/templates/completion-report.md` 準拠):**
    - タスク実装完了時、Workerは `.regista/templates/completion-report.md` に準拠した超凝縮Markdown形式のみでチャット出力する。
    - **YAMLブロックおよび変更ファイル一覧の完全撤廃**: 完了報告の直前に出力されるYAMLブロックや変更ファイルの羅列は無駄な出力であるため【厳格に禁止】とする。ユーザー目線での挙動変化（1〜2行）と確認してほしいこと、検証結果のみを出力して確認待ち状態に移行すること。
    ...
    - **完了報告テンプレート (厳格遵守):**
      ...
      ````
  - **7. ユーザー合図受領後のチケット完了・コミット報告フォーマット (`.regista/templates/ticket-done-report.md` 準拠・義務):**
    - ユーザーから「完了」「OK」等の明示的な合図を受領した後、エージェントは対象チケット（`.regista/tickets/[ID].md`）のステータスを `status: DONE` に更新し、コミットを実行する。
    - **コミット完了後のチャット報告義務**:
      - コミット完了後のチャット出力は、必ず `.regista/templates/ticket-done-report.md` に準拠したフォーマットを使用することを【義務】とする。
      - **【厳格禁止】「実施内容まとめ」等の冗長出力の禁止**: 「実施内容まとめ」「ファイル分割リスト」「テスト結果の再掲」などの冗長な解説・思考プロセスの出力は【一切不要・厳格に禁止】。コミットとチケット更新（DONE）の確認情報のみを出力して即座に完了すること。
    - **チケット完了報告テンプレート (厳格遵守):**
      ````markdown
      ...
      ````
  - **8. 手詰まり時の引継ぎプロトコル (Handover Protocol - キーワード: 「引継ぎ」):**
    - **背景と目的:** Flash (Low/Medium) 等の軽量モデルで実装中、仕様の齟齬や要件調整が重なって手詰まりになり、会話が長引いてコンテキスト（トークン）を無駄に浪費してしまう課題を解消する。会話の要点をファイルに集約・永続化し、新しいチャットでクリーンなコンテキストから即座に続きを再開できるようにする。
    - **発動条件 (キーワード):**
      - ユーザーが「**引継ぎ**」「**引き継ぎ**」「**引き継いで**」と指示した場合。
      - またはエージェント側で仕様のズレ・手詰まりによるコンテキスト浪費を検知し、引き継ぎを提案して合意を得た場合。
    - **実行プロトコル (3ステップ):**
      1. **要約とドキュメント作成 (`.regista/templates/handover.md` 準拠):**
         - これまでの対話内容（経緯、試行錯誤、手詰まり理由、ユーザーとの対話で確定した前提概念・確定仕様、現時点の実装・テスト状態、残課題）を要約する。
         - `.regista/handoffs/{Ticket ID}-handoff.md` にMarkdownファイルとして保存する。
      2. **新規チャット用プロンプトの埋め込み:**
         - 別チャット（新規Conversation）にそのまま貼り付けて再開できるよう、**「チケットID」「引継ぎファイルの絶対パス」「関連ファイル」「確定仕様」「直近着手アクション」** を含む自己完結したプロンプトを生成する。
      3. **チャットへの超凝縮引継ぎ報告 (`.regista/templates/handover-report.md` 準拠):**
         - チャット上には、チケットID、要点をまとめた引継ぎファイルパス（絶対パスリンク）、手詰まり理由、推奨モデル、およびコピーして新規チャットに貼るプロンプトのみを出力する。
         - 手詰まり解消のため、引き継ぎ先の推奨モデルとして `Gemini 3.8 Flash (High)` または `Gemini 3.1 Pro` への昇格を提案する。
    - **引継ぎ報告テンプレート (厳格遵守):**
      ````markdown
...
- **巨大ファイルのピンポイント読み込み**: 200行を超えるファイルに対して無差別な全読み込みを行わない。`grep_search` や `view_file` の `StartLine`/`EndLine` を活用して必要な箇所のみをピンポイントで取得する。
...
- **CLI Output Optimization:** Mermaid 図などの視覚的ダイヤグラムを出力しない。チケット実装完了時は冗長なコード解説・内部実装・思考プロセスの出力を原則禁止とし、YAMLブロックや変更ファイル一覧を含まない超凝縮完了報告フォーマットのみを出力してユーザー確認待ちに移行する。ユーザー合図受領後のチケット完了時も「実施内容まとめ」等の冗長出力を禁止し、チケット更新とコミットの確認のみを出力する。
- **Silent Execution & Indicator:** 作業中のコマンド実況中継（「〇〇を実行します」「テストを走らせます」等）を完全禁止する。ユーザーは実行コマンドの過程ではなく結果を待っている。タスク着手時は `⏳ [RUNNING] {Task/Ticket}` のインジケーター1行のみを表示し、実行中はバックグラウンドで静かに処理を進め、結果のみを報告すること。
...
## 11. Responsibility Segregation (責務の分離) と循環参照の防止
...
- **行動指針:**
    ...
    - **マイクロ検証の徹底:** 複数ファイルを変更する際は、1ファイル（または1サブモジュール）変更するごとに `biome`, `type-check:scoped`, `vitest` を実行し、局所的な安全を確保してから次へ進むこと。一括変更による巨大なエラーログの発生を防ぐ。
...
## 17. 厳格なファイル生成制約と Type & Skeleton-First Driven Development (TSFDD)
- **原則:** 一度の出力で巨大なファイル（ロジックとUIが混在した状態）を生成することを固く禁じる。トークン効率と拡張性のために「Type & Skeleton-First」を徹底する。
- **Tool Call のセルフ・ガードレール (Pre-tool hook 概念):**
    - エージェントが `write_to_file` や `replace_file_content` などのツールを用いてコードを出力する際、**内部的・概念的なPre-tool hook** を適用すること。
    - 生成・置換しようとしている `Content` の行数が **200行（コンポーネント）または 150行（フック）** を超える場合、エージェントは自発的にツール実行を中断（エラー）し、「責務ごとにファイルを分割して再生成する」ループを回すこと。
- **テンプレート・スケルトンの先行配置 (Obligatory):**
    - 新規作成・大規模分割時は、いきなり完全な実装コードを書き込んではならない。
    - **Step 1:** まず、空のディレクトリ構造と「型定義ファイル」「インターフェース」「空の関数スタブ」「空のReactコンポーネント」のみが書かれた**骨組み（スケルトン）ファイル群**を先行配置する。
    - **Step 2:** この骨組みのみの段階で `pnpm type-check:scoped` などを実行し、ファイル間の依存関係とアーキテクチャ・型の整合性を完全に保証する。
    - **Step 3:** 型の安全性が確認されてから、初めて別のステップで各ファイルの中身（ロジックやUIの詳細）を個別に埋めていく。
    - ユーザーから「新規機能を作れ」と指示された場合は、必ずこの「スケルトン先行配置」のアプローチを採用し、「ファイルの中身を埋める準備ができました」と報告（または自動進行）すること。
- **エージェントの作業規約（Self-Scaffolding）**:
    - 3ファイル以上または合計200行を超える実装を行う場合、**コード生成前に作成・修正予定の「ファイル一覧とそれぞれの責務（1行）」を箇条書きで宣言**すること。
    - 一度に全ファイルを出力せず、型定義（`types.ts`）→ ロジック（`hooks/`）→ UI（`components/`）の順で段階的にファイルを作成すること。
