...
- **ツール入力制約の厳格化 (view_file & 検索)**: 
  - **view_file利用の原則禁止と代替ツールの徹底**: トークン消費（全体の約70%）を削減するため、ファイル内容の取得には原則として `rtk smart <file>` (AST/インターフェース抽出) または `rtk read <file>` を使用すること。`view_file` の使用は最終手段とする。
  - **view_file スライス指定の強制**: やむを得ず `view_file` を使用する場合は、引数なしでの全行読み込みを**絶対禁止**とする。必ず `StartLine` と `EndLine` を指定し、1回の読み込みを最大50行以内のチャンクに制限すること。
...
- **コマンド出力の徹底的なサイレント化 (/dev/null の活用)**: `run_command` によるトークン消費を抑制するため、`git status`, `git add` などのシェルコマンドを実行する際、結果の詳細が不要な場合は**必ず `> /dev/null 2>&1` を付与**してコンテキストへの出力を完全に捨てること。結果が必要な場合でも、`rtk` を経由するか `git diff --stat` 等を用いて出力を最小限（数行）に留めること。エージェント自身の履歴を巨大なターミナルログで汚染してはならない。
