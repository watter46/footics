...
- **チケット出力・運用プロトコル (DAG & Markdown Distribution Standard):**
  ...
  - **6. チケット実装完了時の超凝縮チャット報告フォーマット (`.regista/templates/completion-report.md` 準拠):**
    ...
    - 冗長なコード解説や思考プロセスの出力を【原則禁止】とし、ユーザー目線での挙動変化（1〜2行）と確認してほしいこと、変更ファイル、検証結果のみを出力して確認待ち状態に移行すること。
...
- **CLI Output Optimization:** Mermaid 図などの視覚的ダイヤグラムを出力しない。チケット実装完了時は冗長なコード解説・内部実装・思考プロセスの出力を原則禁止とし、超凝縮完了報告フォーマットのみを出力してユーザー確認待ちに移行する。ユーザー合図受領後のチケット完了時も「実施内容まとめ」等の冗長出力を禁止し、チケット更新とコミットの確認のみを出力する。
