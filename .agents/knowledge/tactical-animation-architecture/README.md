...
### 1.16 モバイル最適化・ボトムシートUI・ピッチ常駐ミニコントローラー & 写真フォールバック
- **モジュール:** `tactical-animation-board.tsx`, `animation-timeline-panel.tsx`, `animation-inspector-panel.tsx`, `animation-marker.tsx`, `pitch-renderer.ts`
- **仕様:**
  - **ピッチ常駐ミニプレイヤーバー:** タイムラインを開かずにピッチを見ながらワンタップで「再生/停止」「前/次シーン切替」「シーン追加」「タイムライン/設定トグル」が行えるスリムなフローティングバーをピッチ下部に常設。
  - **再生時自動最小化 (Auto-minimize on Play):** 再生開始時にモバイル画面ではボトムシート（タイムライン/設定）を自動で閉じ、アニメーションの動きを全画面で視認可能に。
  - **ボトムシート化 & バックドロップ:** モバイル画面幅（`< 768px`）ではタイムラインおよびインスペクターが画面下部から引き出すボトムシート（`h-[65vh]`）として開き、背景タップで閉じるバックドロップと排他制御を導入。
  - **ピッチ上下見切れ防止 (Safe Area Resizing):** 下部ミニバー（約58px）と上下左右パディングを差し引いた正確なアスペクト比（縦70:107 / 横107:70）でリサイズ計算を行い、ゴール・ペナルティエリアの見切れを完全に防止。
  - **選手写真フォールバック:** マーカーが「顔写真」モードに設定されていても、画像未登録や読み込み前の選手には自動で背番号をフォールバック描画し、無地の空円になるのを防止（Konvaおよび動画エクスポート両対応）。
