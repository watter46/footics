...
2. **適応型オーケストレーション (Adaptive Routing)** による調査の実行:
   - **局所タスク (対象3ファイル以内・変更箇所既知)**: Scoutを起動せず、GM自身が直接ファイル読み取りを行い即座にチケット化する。
   - **探索型タスク (影響範囲不明・DBスキーマ/型変更など横断調査が必要)**: `regista-scout` に調査を委譲する。
   - ※ 委譲時のプロンプトは【超軽量化 (Compact Protocol)】し、`{"target": "対象", "objective": "目的", "constraints": "制約"}` のようなJSONまたはKey-Value形式のみで送信すること。自然言語での依頼や挨拶は厳禁。
3. 開発ドメイン(A-E)を決定する
4. 担当エージェントをアサインする
5. タスクを極小AAWU（1〜3ファイル単位）に分解し、並列実行可能なDAG構造をJSON等の構造化メタデータとして出力する。長文のMarkdownチケット（`.regista/tickets/[ID].md`）の直接出力は廃止し、生成・保存は軽量サブエージェント `regista-ticket-writer` へ委譲する（※委譲時も軽量JSONのみで通信すること）。

**Exit Criteria**: 本質課題が1文で記述 / 影響ファイル特定済 / ドメイン決定 / 担当決定 / `.regista/tickets/` へのチケット発行完了 / **`pnpm tickets:validate` エラーなし**
...
GMの責務は以下の2点のみです：
1. タスクをレイヤー分割してJSONメタデータで出力する
2. チケット保存後に `rtk pnpm tickets:validate` を実行しバリデーションを通過させる
...
※ CLIから `pnpm tickets` を実行することで最新のチケット状況を即座に確認可能。
