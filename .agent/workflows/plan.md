...
## 2. 成果物生成（"Brain" プロトコル）
- **目的:** 計画をエージェントのメモリ構造に永続化し、Antigravity の機能として統合する。
- **厳格な要件:**
  - 適切な計画ディレクトリ内のマークダウンファイルを作成または更新しなければなりません。
  - **重要:** `write_to_file` 呼び出し時には必ず **`IsArtifact: true`** を設定してください。
  - **ArtifactMetadata**: `ArtifactType` に `"implementation_plan"` または `"task"` を指定し、`Summary` には詳細な要約を記述してください。
- **ディレクトリの安全性:**
...
3.  **提案される変更:**
    ...
    - **重要:** 必ず `./.agent/skills/quality-harness/` スキルを適用し、`task.md` の各ステップに検証アクション（`Verify with quality-harness`）を含めてください。
4.  **実装ステップと粒度:**
    - タスクを独立して検証可能な小さなステップ（目安として 1ファイル 1機能）に分解してください。
    - 各ステップに明確な作業内容と、その検証方法を記述してください。
5.  **リスク評価:** 回避すべき潜在的な副作用やハルシネーション。
