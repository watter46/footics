---
trigger: always_on
globs: ["src/hooks/**/*.{ts,tsx}", "src/lib/**/*.{ts,tsx}", "extension/**/*.{ts,tsx}"]
---

# Data Management Patterns (データ管理パターン)

**Activation:** This rule is **ALWAYS ON** for files matching `src/hooks/**/*.{ts,tsx}`, `src/lib/**/*.{ts,tsx}`, and `extension/**/*.{ts,tsx}`.
...
- **全領域での標準化:** Web本体（`src/`）および拡張機能（`extension/`）の双方において、IndexedDB の操作には `Dexie.js` を統一して使用する（`idb-keyval` 等の利用は撤廃）。
- **レイヤー分離:** Dexie インスタンスの直接操作を極力避け、原則として `src/lib/db.ts` や専用サービスクラス等の統一レイヤーを介する。
