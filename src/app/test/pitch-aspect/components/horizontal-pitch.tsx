...
function HorizontalPitchGrass({ geom }: { geom: PitchLayoutGeometry }) {
  const stripeWidth = geom.pitchWidth / 18;
  return (
    <>
      ...
      {Array.from({ length: 18 }).map((_, i) => (
        <rect
          ...
          key={`h-stripe-${i}`}
          x={geom.pitchLeft + i * stripeWidth}
          y={geom.pitchTop}
          width={stripeWidth}
          height={geom.pitchHeight}
...
function HorizontalPenaltyAreas({
  geom,
  strokeColor,
  strokeWidth,
  strokeOpacity,
  ...
  return (
    <>
      ...
      />
      <rect
        x={geom.pitchLeft + geom.pitchWidth - geom.paDepth}
        y={geom.centerY - geom.paWidth / 2}
        width={geom.paDepth}
        height={geom.paWidth}
        fill="none"
      ...
      />
      <rect
        x={geom.pitchLeft + geom.pitchWidth - geom.gaDepth}
        y={geom.centerY - geom.gaWidth / 2}
        width={geom.gaDepth}
        height={geom.gaWidth}
        fill="none"
      ...
      {/* ペナルティスポット */}
      <circle
        cx={geom.pitchLeft + geom.penaltySpotDist}
        cy={geom.centerY}
      ...
      />
      <circle
        cx={geom.pitchLeft + geom.pitchWidth - geom.penaltySpotDist}
        cy={geom.centerY}
        r={0.3 * geom.meterScale}
        fill={strokeColor}
        opacity={strokeOpacity}
      ...
      {/* ペナルティアーク */}
      <path
        d={`M ${geom.pitchLeft + geom.paDepth} ${geom.centerY - arcOffset} A ${r} ${r} 0 0 1 ${geom.pitchLeft + geom.paDepth} ${geom.centerY + arcOffset}`}
        fill="none"
        stroke={strokeColor}
        strokeWidth={strokeWidth}
        strokeOpacity={strokeOpacity}
      />
      <path
        d={`M ${geom.pitchLeft + geom.pitchWidth - geom.paDepth} ${geom.centerY - arcOffset} A ${r} ${r} 0 0 0 ${geom.pitchLeft + geom.pitchWidth - geom.paDepth} ${geom.centerY + arcOffset}`}
        fill="none"
        stroke={strokeColor}
        strokeWidth={strokeWidth}
        strokeOpacity={strokeOpacity}
...
function HorizontalCenterCircle({
  geom,
  strokeColor,
  strokeWidth,
  strokeOpacity,
  ...
  return (
    <>
      ...
      />
      {showCircleRuler && (
        <g id="circle-ruler-h" className="no-export" opacity={0.75}>
          <line
            x1={geom.centerX - r}
            y1={geom.centerY}
            x2={geom.centerX + r}
...
export function HorizontalPitch({
  geom,
  strokeColor = '#e2b48d',
  strokeOpacity = 0.85,
  strokeWidth = 4,
  ...
  return (
    <g id="horizontal-pitch">
      <HorizontalPitchGrass geom={geom} />
      {/* 外枠タッチライン・ゴールライン */}
      <rect
        x={left}
        y={top}
