...
import { FormationMode } from '@/lib/data/formations';
...
export const TacticalBoardModal: React.FC<TacticalBoardModalProps> = ({
  ...
  const { 
    savedSettings, activeId, 
    benchTeam, setBenchTeam,
    homeColor, awayColor 
  } = useTacticalStore();
          ...
          <div className="flex-1 flex flex-row gap-4 p-4 bg-slate-950/20 overflow-hidden relative">
            ...
            <div className="flex-[1] min-w-0 h-full flex flex-col min-h-0">
              <BenchArea 
                ...
                onTeamToggle={() => setBenchTeam(benchTeam === "home" ? "away" : "home")}
                onAlignGrid={handleAlignBench}
                formationMode={formationMode}
                onFormationModeChange={setFormationMode}
                onFormationChange={(type) => handleApplyFormation(benchTeam, type, formationMode)}
              >
                {benchPlayers.map((p) => {
